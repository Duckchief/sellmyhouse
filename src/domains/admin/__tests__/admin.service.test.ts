...
describe('getAdminSellerDetail', () => {
  beforeEach(() => {
    mockAgentService.getNotificationHistory.mockResolvedValue({ items: [], total: 0, page: 1, totalPages: 0 });
  });
  ...
  it('returns full detail when seller exists', async () => {
    ...
    mockAgentService.getNotificationHistory.mockResolvedValue({ items: baseNotification, total: 1, page: 1, totalPages: 1 } as never);
