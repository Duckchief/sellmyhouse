...
export const SETTING_VALIDATORS: Record<SettingKey, (v: string) => boolean> = {
  ...
  sensitive_doc_retention_days: (v) => Number.isInteger(Number(v)) && Number(v) >= 1 && Number(v) <= 30,
  financial_data_retention_days: (v) => Number.isInteger(Number(v)) && Number(v) >= 1 && Number(v) <= 30,
  transaction_anonymisation_days: (v) => Number.isInteger(Number(v)) && Number(v) >= 7 && Number(v) <= 90,
