...
export function buildFinancialNarrativePrompt(
  ...
  sections.push(
    `- **Estimated Net Cash Proceeds: $${formatCurrency(output.netCashProceeds)}**`,
  );
  ...
  sections.push(
    '- Explain each deduction briefly so the seller understands where the money goes',
  );
  ...
  sections.push(
    '- Do NOT use technical jargon — explain terms like "accrued interest" simply',
  );
